...
AddNamespace("NPR.responsivePage");NPR.responsivePage.trackWidth=0;NPR.responsivePage.ieEightCheck=($.browser.msie===true);NPR.responsivePage.renderMobileDisqus=false;NPR.responsivePage.stickyEggCartonOffset=360;$(document).ready(function(){NPR.responsivePage.initContentHeader();NPR.responsivePage.initGlobalFooter();NPR.responsivePage.initializeContentFooter();NPR.responsivePage.initSearchToggle();NPR.responsivePage.initCommentToggle();NPR.responsivePage.initAudioHover();NPR.responsivePage.initEcommercePop();NPR.responsivePage.initFooterLinks();NPR.responsivePage.trackWidth=$(window).width();NPR.responsivePage.initOffCanvasEvents();NPR.responsivePage.positionShareToolsAnd88ad();if((NPR.Devices.isScreenLargerOrEqual(NPR.BREAKPOINT.LARGE)===true||NPR.Devices.isScreenWithin(NPR.BREAKPOINT.MEDIUM_WITH_AD))&&(!$('body').hasClass('agg')&&!$('body').hasClass('multimedia'))){NPR.responsivePage.positionEggCarton();NPR.responsivePage.stickyEggCartonCalc();}
...
if($('html').hasClass('lt-ie9')&&$('.story #resMore').length>0){$("#resMore").appendTo("#main_sidebar");return;}};NPR.responsivePage.stickyEggCartonCalc=function(){try{if($('#relatedstorieswrap').length>0)
{var stickyEggCartonOffset=$('#relatedstorieswrap').offset().top;if(stickyEggCartonOffset>=360){return NPR.responsivePage.stickyEggCartonOffset=stickyEggCartonOffset;}}}
catch(e){NPR.messaging.exception(e,'responsive_page.js','NPR.responsivePage.stickyEggCartonCalc');}};NPR.responsivePage.stickyEggCarton=function(){if($('.adwrapper.housead').length>0){var windowTop=$(window).scrollTop();var houseAdOffset=$('.adwrapper.housead').offset().top;var stickyEggCartonHeight=$('#relatedstorieswrap').height();var stopPoint=houseAdOffset-stickyEggCartonHeight;var stoppedBottomMargin=20;if(windowTop>=NPR.responsivePage.stickyEggCartonOffset&&(NPR.Devices.isScreenLargerOrEqual(NPR.BREAKPOINT.LARGE)===true||NPR.Devices.isScreenWithin(NPR.BREAKPOINT.MEDIUM_WITH_AD)===true)){$('#relatedstorieswrap').addClass('sticky').removeClass('stopped').css('top','');if(windowTop>(stopPoint-stoppedBottomMargin)&&!$('body').hasClass('agg')){$('#relatedstorieswrap').removeClass('sticky').addClass('stopped');$('#relatedstorieswrap').css({'top':stopPoint-stoppedBottomMargin-49});}}else{$('#relatedstorieswrap').removeClass('sticky').removeClass('stopped').css('top','');}}};NPR.responsivePage.positionShareToolsAnd88ad=function(){if(!$('body').hasClass('agg')&&!$('html').hasClass('lt-ie9')&&!$('body').hasClass('multimedia')){var $widgetsToMove=$('.story .social, #ad_impression_88_wrap');if(NPR.Devices.isScreenLargerOrEqual(1448)===true){var contentWidth=$('#wrapper').width();var viewportWidth=$(window).width();var nonContentSpace=viewportWidth-contentWidth;var amountToOffset=(nonContentSpace/2)+$('#main_sidebar').width()+2;$widgetsToMove.css('right',amountToOffset);}else{$widgetsToMove.css('right','');}}};NPR.responsivePage.initFooterLinks=function(){if($('html').hasClass('touch')){$('#footerContact a').attr('href','http://help.npr.org/npr/wf/m/index.aspx');}};
...
$(document).ready(NPR.enhancements.bindEnhancements);